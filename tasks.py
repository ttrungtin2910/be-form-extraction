...
def upload_image_task(
    ...
    Args:
        ...
        upload_by: Username who uploaded the image.
    ...
    # Log the very first line to confirm task is being called
    logger.info("=" * 80)
    logger.info(f"[upload_image_task] TASK STARTED!")
    logger.info(f"[upload_image_task] All parameters received:")
    logger.info(f"  - temp_local_path: {temp_local_path}")
    logger.info(f"  - original_filename: {original_filename}")
    logger.info(f"  - status: {status}")
    logger.info(f"  - folder_path: {folder_path}")
    logger.info(f"  - upload_by: '{upload_by}'")
    logger.info("=" * 80)

    try:
        # Debug: Log upload_by parameter
        logger.info(f"[upload_image_task] Received upload_by parameter: '{upload_by}'")

        ext = os.path.splitext(original_filename or "")[1].lower() or ".jpg"
        image_name = _timestamp_name(ext)
        destination_blob_name = (
            f"{folder_path}/{image_name}" if folder_path else image_name
        )
        ...
        # Debug: Log the ImageData being saved
        logger.info(
            f"[upload_image_task] Saving ImageData with UploadBy: '{upload_by}'"
        )
        logger.info(f"[upload_image_task] Full meta: {meta.dict()}")

        upsert_image(meta, _config.COLLECTION_NAME_IMAGE_DETAIL, meta.ImageName)
        logger.info(f"[upload_image_task] ImageData saved successfully")
        try:
    ...
    try:
        ...
        try:
            ...
            processing_meta = ImageData(
                ...
                UploadBy=existing.get("UploadBy", ""),  # Preserve UploadBy from upload
            )
            upsert_image(
                processing_meta, _config.COLLECTION_NAME_IMAGE_DETAIL, image_name
            )
        ...
        if not size:
            if "existing" not in locals():
                existing = (
                    get_image(image_name, _config.COLLECTION_NAME_IMAGE_DETAIL) or {}
                )
        ...
        # Update status in imagedetail - preserve UploadBy
        existing_image = (
            get_image(image_name, _config.COLLECTION_NAME_IMAGE_DETAIL) or {}
        )
        meta = ImageData(
            ...
            FolderPath=folder_path,
            Size=size,
            UploadBy=existing_image.get(
                "UploadBy", ""
            ),  # Preserve UploadBy from upload
        )
